...
def require_user(x_user_id: Optional[str]):
    if not x_user_id:
        raise HTTPException(status_code=401, detail="Missing X-User-Id header")
    return x_user_id

@router.post("/sightings", response_model=SightingOut)
def post_sighting(payload: SightingCreate, db: Session = Depends(get_db), x_user_id: Optional[str] = Header(None, convert_underscores=False)):
...
def get_my_sightings(since_days: int = Query(365, ge=1, le=3650), db: Session = Depends(get_db), x_user_id: Optional[str] = Header(None, convert_underscores=False)):
    user_id = require_user(x_user_id)
