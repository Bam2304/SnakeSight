...
class DB:
    def __init__(self):
        self.supabase_url: str = "https://lgvelnkhepuokjbacqqx.supabase.co" #os.getenv("SUPABASE_URL")
        self.supabase_key: str = "sb_publishable_VwY13z1yliMQ698Km0fNYA_tgpwqZab" #os.getenv("SUPABASE_KEY")
        self.supabase: Client = create_client(self.supabase_url, self.supabase_key)
        
    def DataFromDatabase(self):
        #Gets Data From the Main Table in Supabase
        response = (self.supabase.from_("SnakeMain").select("*").execute() )
        data = response.data
        DataMain = response.data


        #Gets Data From the Habits Table in Supabase
        response = (self.supabase.from_("Habits").select("*").execute())
        ...
        response = ( self.supabase.from_("Characteristics").select("*").execute() )
        ...
        response =( self.supabase.from_("OtherNames").select("*").execute())
        ...
        response = (self.supabase.from_("SnakeImages").select("*").execute())
